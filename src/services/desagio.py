"""Cálculo de deságio por título (fator em ``sigbors``) e regras de prazo cedente/sacado."""
...
def _get_sec_connection():
    """Cria conexão com o banco de securitização (``sigbors``)."""
    conn_str = (
...
def calcular_desagio(
    ...
    if coluna_valor_base not in df.columns:
        raise ValueError(
            f"Coluna {coluna_valor_base!r} ausente no DataFrame para cálculo de deságio."
        )
    ...
    base = pd.to_numeric(df[coluna_valor_base], errors="coerce").fillna(0)
